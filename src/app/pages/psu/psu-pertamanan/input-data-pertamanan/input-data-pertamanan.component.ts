...
export class InputDataPertamananComponent implements OnInit {
  ...
  public addDataPetugas() {
    const dataPetugas = this.dataPetugasForm.get('dataPetugas') as FormArray;
    dataPetugas.push(this.createDataPetugasFormGroup())
  }
  ...
  public removeDataPetugas(j: number) {
    const dataPetugas = this.dataPetugasForm.get('dataPetugas') as FormArray;
    if (dataPetugas.length > 1) {
      dataPetugas.removeAt(j)
    } else {
      dataPetugas.reset()
    }
  }
